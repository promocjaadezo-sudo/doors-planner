...
(function(){
  ...
  function mergeLegacyState(targetState){
    ...
    const deletedOrderIds = new Set(Array.isArray(targetState._deletedOrderIds) ? targetState._deletedOrderIds : []);
    ...
    LEGACY_STORE_KEYS.forEach(key => {
      ...
      legacyStates.forEach(legacyState => {
          ...
          if (deletedOrderIds.has(normalizedOrder.id)) return;
        ...
        if (changed) updatedOrders++;
          ...
          (Array.isArray(legacyState?.after) ? legacyState.after : []).forEach(entry => {
            ...
            if (normalizedEntry.order && deletedOrderIds.has(normalizedEntry.order)) return;
  ...
  if (!Array.isArray(state._deletedOrderIds)) state._deletedOrderIds = [];
  ...
  function saveState(shouldSaveToDb = false){ 
    try{ 
      ...
      if (!Array.isArray(state._deletedOrderIds)) {
        state._deletedOrderIds = [];
      } else if (state._deletedOrderIds.length) {
        state._deletedOrderIds = Array.from(new Set(state._deletedOrderIds.filter(Boolean)));
      }

...
(function(global){
  ...
  function generateTasksForOrder(order, state) {
    ...
    process.operations.forEach((op, opIndex) => {
      ...
      console.log(`[generateTasksForOrder] Operacja ${opIndex}: ${op.name}, ID: ${op.id}, dependsOn=${op.dependsOn}, hasDepends=${hasDepends}`);
      
      // Znajdź operację w katalogu aby pobrać checklistę
      const catalogOp = operationsCatalog.find(catOp => catOp.id === op.id);
      console.log(`[generateTasksForOrder] Operacja w katalogu: ${catalogOp ? catalogOp.name : 'NIE ZNALEZIONO'}`);
      ...
      const task = {
        ...
        processId: processId,
        operationId: op.id,
        opId: op.id, // dla kompatybilności z worker-app
        name: op.name,
      ...
      // Kopiuj checklistę z operacji w katalogu jeśli propagateToTasks jest włączone
      if(catalogOp && catalogOp.checklistTemplate && catalogOp.checklistSettings && catalogOp.checklistSettings.propagateToTasks) {
        ...
        if(Array.isArray(template.items) && template.items.length > 0) {
          task.checklist = template.items.map(item => ({
            id: item.id || genId('chk_'),
            label: item.label || '',
            done: false,
            required: item.required || false,
            metadata: item.metadata || {}
          }));
          task.checklistRequired = catalogOp.checklistSettings.required || false;
          task.checklistAllowAdd = catalogOp.checklistSettings.allowWorkerAdditions !== false;
          console.log(`[generateTasksForOrder] ✅ Skopiowano ${task.checklist.length} punktów checklisty z operacji "${op.name}"`);
        } else {
          console.log(`[generateTasksForOrder] ⚠️ Operacja "${op.name}" ma checklistTemplate ale items jest puste`);
        }
      } else {
        console.log(`[generateTasksForOrder] ℹ️ Operacja "${op.name}" nie ma checklisty lub propagateToTasks=false`);
      }
  ...
  function generateSchedule(state, opts){
    ...
    try {
      ...
      if (pending.length > 0) {
        // Użyj scheduleByEmployee dla planowania per pracownik (równolegle)
        const scheduled = scheduleByEmployee(pending, state);
  ...
  /**
   * Planowanie zadań per pracownik - każdy pracownik ma swoją kolejkę
   * Różni pracownicy mogą pracować równolegle
   */
  function scheduleByEmployee(tasks, state){
    const cfg = (state && state.scheduleConfig) || { workdayStartHour:8, workdayLengthHours:8 };
    const offWeekdays = cfg.offWeekdays ? cfg.offWeekdays.map(Number) : [6,0];
    const holidays = Array.isArray(cfg.holidays) ? cfg.holidays : [];
    const startHour = Number(cfg.workdayStartHour)||8;
    const lenH = Number(cfg.workdayLengthHours)||8;
    const endHour = startHour + lenH;
    ...
    function nextWorkingDay(d){
      while(true){
        const wd = d.getDay();
        const isWorkingDay = (!offWeekdays.length || offWeekdays.indexOf(wd) === -1);
        if(isWorkingDay && !isHolidayDate(d)) return d;
        d.setDate(d.getDate()+1);
        d.setHours(startHour,0,0,0);
      }
    ...
    const byEmployee = {};
    tasks.forEach(t => {
      const empId = t.employeeId || 'unassigned';
      if(!byEmployee[empId]) byEmployee[empId] = [];
      byEmployee[empId].push(t);
    });

    console.log('[scheduleByEmployee] Pracownicy:', Object.keys(byEmployee).length, 'Zadania:', tasks.length);

    // Dla każdego pracownika planuj sekwencyjnie
    Object.entries(byEmployee).forEach(([empId, empTasks]) => {
      // Sortuj po seq i zależnościach
      empTasks.sort((a, b) => (a.seq || 0) - (b.seq || 0));
      ...
      empTasks.forEach(t => {
        ...
        console.log(`[scheduleByEmployee] ${empId}: ${t.opName || t.name} -> ${new Date(start).toLocaleString('pl-PL')}`);
      });
    });
  ...
  // Public API
  const api = { computeDuration, generateTasksForOrder, generateSchedule, migrateTasks, scheduleSequential, scheduleByEmployee, computeCriticalPath };
